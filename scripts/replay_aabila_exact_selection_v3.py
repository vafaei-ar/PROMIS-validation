...
OUT=VAL/"results/aabila_exact_replay"
...
def loadmod():
    ...
    def safe_makedirs(path,*args,**kwargs):
        s=str(path)
        if s.startswith("/home/atharzeen/Research"):
            return None
...
def local_input_dir():
    for p in [GEI/"dataset/parquet", GEI/"dataset"]:
        if (p/"encounter.parquet").exists() and (p/"diagnosis.parquet").exists():
            return p
    raise FileNotFoundError("Could not locate local Geisinger encounter.parquet and diagnosis.parquet")

m=loadmod()

# Load Aabila config if possible, but force the input directory to the local copy.
cfg=m.load_config()
inp=local_input_dir()

# Patch common config backends so Aabila's own filter functions read local files.
patched=False
...
try:
    if hasattr(cfg,"set"):
        cfg.set("data_paths.input_data_dir",str(inp)); patched=True
except Exception:
    pass
...
for d in (en,dx):
    d["PATID"]=norm(d["PATID"]); d["ENCOUNTERID"]=norm(d["ENCOUNTERID"])
...
if "ENC_TYPE" in dx.columns:
    dx=dx[dx["ENC_TYPE"].isin(crit["encounter_types"])]
...
if crit["require_primary_diagnosis"]:
    if "DX_PRIMARY_YN" in sdx.columns:
        sdx=sdx[sdx["DX_PRIMARY_YN"].astype(str).str.strip().isin(["1","1.0","Y","YES","TRUE"])].copy()
    elif "PDX" in sdx.columns:
        sdx=sdx[sdx["PDX"].astype(str).str.strip().str.upper().isin(["P","1","Y","YES","TRUE"])].copy()
...
cand=m.apply_imaging_filter(cand,cfg); n1=(len(cand),cand["PATID"].nunique())
cand=m.apply_lipid_filter(cand,cfg); n2=(len(cand),cand["PATID"].nunique())
...
try:
    ...
    print(f"WARNING: exact age-filter replay failed: {e}")
...
cmp=saved[["PATID","ENCOUNTERID"]].drop_duplicates("PATID").rename(columns={"ENCOUNTERID":"saved"}).merge(
    first[["PATID","ENCOUNTERID"]].drop_duplicates("PATID").rename(columns={"ENCOUNTERID":"replay"}),on="PATID",how="outer")
...
diag_cols=[c for c in [
    "PATID","ENCOUNTERID","DX_DATE_stroke","ADMIT_DATE","DISCHARGE_DATE",
    "ENC_DT","ENC_ADM_DTTM","DX","HAS_IMAGING","HAS_LIPID",
    "has_neuroimaging","has_lipid_panel"
] if c in r.columns]
...
saved_dates=saved[["PATID","ENCOUNTERID"] + ([c for c in ["DX_DATE_stroke","ADMIT_DATE","DISCHARGE_DATE"] if c in saved.columns])].copy()
saved_dates=saved_dates.rename(columns={"ENCOUNTERID":"saved"})
replay_dates=first[["PATID","ENCOUNTERID"] + ([c for c in ["DX_DATE_stroke","ADMIT_DATE","DISCHARGE_DATE"] if c in first.columns])].copy()
replay_dates=replay_dates.rename(columns={"ENCOUNTERID":"replay"})
md=cmp[cmp["saved"].notna() & cmp["replay"].notna() & ~cmp["match"]][["PATID","saved","replay"]]
md=md.merge(saved_dates,on=["PATID","saved"],how="left",suffixes=("","_saved"))
md=md.merge(replay_dates,on=["PATID","replay"],how="left",suffixes=("_saved","_replay"))
md.to_csv(OUT/"mismatch_summary.csv",index=False)
...
if edge.exists():
    ...
    edge_n=int(ec["PATID"].nunique()); edge_match=int(ec["match"].sum())
    ec.to_csv(OUT/"edge_encounter_comparison.csv",index=False)

summary={
 "local_input_dir":str(inp),
 "config_input_dir_patched":patched,
 ...
 "saved_and_replay_encounter_mismatches":int(
     (cmp["saved"].notna() & cmp["replay"].notna() & cmp["saved"].ne(cmp["replay"])).sum()
 ),
